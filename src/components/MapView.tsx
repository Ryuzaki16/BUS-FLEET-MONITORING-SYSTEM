...
interface MapViewProps {
  ...
  height?: string
  routePoints?: [number, number][]   // optional dynamic route from parent
}
...
export function MapView({
  ...
  // Initialize map only once, centered on user's GPS location
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      import('leaflet').then((L) => {
        ...
        try {
          // Default fallback center (Manila) if GPS is denied
          ...
          if (navigator.geolocation) {
            navigator.geolocation.watchPosition(
              (position) => {
                ...
                // Re-center map on first GPS fix
              ...
              (error) => {
                ...
                // Stay on fallback center, no user marker shown
              },
              {
  ...
  // Draw/update dynamic route lines when routePoints prop changes
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      const shadow = L.polyline(routePoints, {
        color: '#000000', weight: 10, opacity: 0.1,
      ...
      routeLinesRef.current = [shadow, backing, main, dashed, startMarker, endMarker]
    })
  }, [routePoints])

  // Update bus markers when bus data changes
  useEffect(() => {
    if (!mapInstanceRef.current || typeof window === 'undefined') return
          ...
          const busIcon = L.divIcon({
            html: `
              <div style="position: relative; cursor: pointer;">
                ...
                ${isSelected ? `<div style="position: absolute; inset: -${isMobile ? '6px' : '8px'}; border-radius: 50%; background: rgba(99,102,241,0.3); animation: ping 1s cubic-bezier(0,0,0.2,1) infinite;"></div>` : ''}
              </div>
              ...
              <div style="padding: 0 ${isMobile ? '3px' : '4px'};">
                ...
                <div style="margin-bottom: ${isMobile ? '4px' : '6px'};">
                  ...
                  <div style="width: 100%; height: ${isMobile ? '4px' : '6px'}; background: #e5e7eb; border-radius: ${isMobile ? '2px' : '3px'}; overflow: hidden;">
                    <div style="width: ${occupancyPercent}%; height: 100%; background: ${colorClass};"></div>
                  </div>
                </div>
